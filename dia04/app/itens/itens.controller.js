(function () {
    ...
    itensController.$inject = ["helperFactory", "itensService", "$routeParams"];
    ...
    function itensController(helper, service, $routeParams) {
        ...
        function iniciar() {
            if ($routeParams.id) {
                ...
                return listar().then(listarCategorias);
            }
        }
        ...
        function consultar(_id) {
            return service.consultar(_id).then(tratarRetorno);

            function tratarRetorno(_resp) {
                if (_resp._id) {
                    return _resp;
                } else helper.addMsg(_resp.msg, "danger", "Tente novamente");
            }
        }
        ...
        function deletar(_id) {
            return service.deletar(_id).then(tratarRetorno);
        ...
        /* ***************    FUNÇÕES INSTERNAS    ******************************** */
        function listar() {
            return service.listar().then(salvarItens);
        ...
        function listarCategorias() {
            return service.listarCategorias().then(salvarCategorias);
        ...
        function cadastrar() {
            return service.cadastrar(vm.item).then(function (_item) {
                tratarResposta("cadastrado", _item);
            });
        }

        function editar() {
            return service.editar(vm.item).then(function (_item) {
        ...
        function tratarResposta(_acao, _item) {
            ...
            } else helper.addMsg(null, "danger", "Tente novamente");
        }
    }
})();
